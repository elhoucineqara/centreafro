    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight mb-1">مرحباً بك مجدداً، أيها المشرف</h2>
                    <p className="text-slate-500 font-bold text-xs sm:text-sm active:text-blue-600 transition-colors cursor-default">
                        إليك نظرة شاملة على نشاط المركز.
                    </p>
                </div>
                <div className="flex items-center gap-2.5 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100 self-start">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] sm:text-xs font-black text-slate-900">{new Date().toLocaleDateString('ar-MA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'طلب قيد الانتظار', value: members.filter(m => m.status === 'pending').length, color: 'amber', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
                    { label: 'عضو مقبول', value: members.filter(m => m.status === 'approved').length, color: 'emerald', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
                    { label: 'طلب مرفوض', value: members.filter(m => m.status === 'rejected').length, color: 'rose', icon: 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z' }
                ].map((stat, i) => (
                    <motion.div
                        key={i}
            ...
            <div className="bg-white rounded-[1.5rem] sm:rounded-[3rem] shadow-2xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
                ...
                <div className="w-full">
                    {loading ? (
                        ...
                        <div className="py-20 sm:py-32 text-center px-6">
                            ...
                            <p className="text-slate-500 font-black text-lg sm:text-xl mb-4">لا توجد نتائج مطابقة</p>
                            <button onClick={() => setSearch('')} className="px-6 py-2 bg-blue-50 text-blue-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">إعادة تعيين البحث</button>
                        </div>
                    ) : (
                        <>
                            {/* Desktop View Table */}
                                ...
                                <table className="w-full text-right border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50/30 border-b border-slate-100">
                                            ...
                                            <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">الوضع المهني</th>
                                        ...
                                        {members.map((member, i) => (
                                            ...
                                            >
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className="relative shrink-0">
                                                            <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center font-black text-slate-400 group-hover:scale-110 transition-transform duration-500 shadow-inner overflow-hidden relative border-2 border-transparent group-hover:border-blue-500/20">
                                                                {member.photoUrl ? (
                                                            ...
                                                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-[2px] border-white shadow-sm ${member.status === 'approved' ? 'bg-emerald-500' :
                                                                member.status === 'rejected' ? 'bg-rose-500' : 'bg-amber-500'
                                                                }`} />
                                                        </div>
                                                        <div>
                                                            <div className="text-base font-black text-slate-900 mb-0.5 tracking-tight group-hover:text-blue-600 transition-colors uppercase">{member.fullNameArabic}</div>
                                                            <div className="text-[9px] text-slate-400 font-black flex items-center gap-1.5 uppercase tracking-widest leading-none mb-1 mt-1">
                                                ...
                                                <td className="px-6 py-4">
                                                    {member.employed ? (
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-[9px] px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-lg font-black border border-emerald-100 self-start uppercase">{member.sector}</span>
                                                        </div>
                                                    ) : (
                                                        <span className="text-[10px] text-slate-300 italic font-bold uppercase tracking-widest">غير موظف</span>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className={`inline-flex px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter border
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <button onClick={() => handlePrintView(member)} className="p-2 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-900 hover:text-white transition-all" title="مشاهدة">
                                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div className="lg:hidden divide-y divide-slate-100">
                                {members.map((member, i) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="relative shrink-0">
                                                <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center font-black text-slate-400 shadow-inner overflow-hidden relative">
                                                    {member.photoUrl ? (
                                                        <Image src={member.photoUrl} alt="" fill className="object-cover" unoptimized />
                                                    ) : (
                                                        member.fullNameFrench.charAt(0)
                                                ...
                                                <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-[3px] border-white shadow-sm ${member.status === 'approved' ? 'bg-emerald-500' :
                                                    member.status === 'rejected' ? 'bg-rose-500' : 'bg-amber-500'
                                                    }`} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="text-lg font-black text-slate-900 mb-0.5 tracking-tight truncate uppercase leading-tight">{member.fullNameArabic}</div>
                                                <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest leading-none mb-1">{member.fullNameFrench}</div>
                                                <div className="text-[10px] text-blue-500/70 font-black lowercase tracking-wider leading-none truncate">{member.email}</div>
                                            </div>
                                            <div className={`shrink-0 px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter border
                                                ${member.status === 'approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                                                    member.status === 'rejected' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                                                        'bg-amber-50 text-amber-600 border-amber-100'}`}
                                            >
                                                {member.status === 'approved' ? 'مقبول' :
                                                    member.status === 'rejected' ? 'مرفوض' : 'متظر'}
                                            </div>
                                        </div>

                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">
                                                <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest mb-1">المؤهل</p>
                                                <p className="text-[10px] font-black text-slate-800 line-clamp-1">{member.educationLevel}</p>
                                            </div>
                                            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">
                                                <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest mb-1">التخصص</p>
                                                <p className="text-[10px] font-black text-slate-800 line-clamp-1">{member.specialization}</p>
                                            </div>
                                        </div>
                ...
                {!loading && totalPages > 1 && (
                    <div className="px-5 sm:px-6 py-4 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                            <button
                                ...
                                className="flex-1 sm:flex-initial px-6 py-2 bg-white border border-slate-200 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] text-slate-700 hover:bg-slate-50 disabled:opacity-40 transition-all font-tajawal"
                            >
                                السابق
                            </button>
                            <button
                                ...
                                onClick={() => setPage(p => p + 1)}
                                className="flex-1 sm:flex-initial px-8 py-2 bg-slate-900 border border-transparent rounded-xl text-[9px] font-black uppercase tracking-[0.2em] text-white hover:bg-black disabled:opacity-40 transition-all shadow-lg font-tajawal"
                            >
                                التالي
                            </button>
                        </div>
